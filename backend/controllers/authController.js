...
exports.getProfile = (req, res) => {
  db.query(
    'SELECT id as id, full_name, email, phone, role, created_at FROM users WHERE id = ?',
    [req.user.id],
    (err, rows) => {
      if (err) return res.status(500).json({ message: err.message });
...
exports.getCustomers = (req, res) => {
  db.query(
    "SELECT id as id, full_name, email, phone, role, created_at FROM users WHERE role = 'customer' ORDER BY created_at DESC",
    (err, rows) => {
      if (err) return res.status(500).json({ message: err.message });
      res.json(rows);
    }
